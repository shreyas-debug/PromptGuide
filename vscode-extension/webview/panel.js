...
(function () {
  ...
  if (diagToggle) {
    diagToggle.addEventListener('change', () => {
      vscode.postMessage({ type: 'toggleDiagnostics', enable: diagToggle.checked });
    });
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
      case 'config':
        ...
        if (msg.enableDiagnostics !== undefined && diagToggle) {
          diagToggle.checked = msg.enableDiagnostics;
        }
